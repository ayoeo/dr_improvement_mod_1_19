...
vec4 colorEnergyBase = vec4(0.8, 0, 0.2, 0.8);
vec4 colorEnergyBase2 = vec4(0.6, 0, 0.15, 0.8);
vec4 colorEnergy = vec4(0, 0.8, 0.55, 0.8);
vec4 colorEnergy2 = vec4(0, 0.7, 0.7, 0.8);

vec4 colorHpBase = vec4(0.8, 0, 0.2, 0.8);
vec4 colorHp = vec4(0, 0.75, 0.6, 0.8);

// TODO - config???!+!!+!+!+???!+!
float healthBarSize = 0.4;
float healthBarOutlineSize = .0025;
float healthBarThickness = 0.01;
float healthBarY = 0.964;
float healthBarMixThreshold = 0.0015;

// TODO - CONFIGGILG?NG?G
float barWidth = 22;
float energyBarThickness = 0.022;
float energyBarOutlineSize = .002;
// bigger is closer lol
float yOffset = 0.12;

// eyeball it baby
float energyWidth = 0.073;
...
float drawEnergyBar(bool flip) {
  ...
  float rb = 0.005;// TODO - change for THICKNESS aka also make thick happy ? maybe

  vec2 offset = vec2(0, yOffset);// TODO - change to OFFSET IT LOL
  return sdArc(vec2(-0.5, -0.5) + uv + (flip ? -offset : offset), vec2(sin(ta), cos(ta)), vec2(sin(tb), cos(tb)), 0.16, rb);// TODO change .12 to make TINY or BIG??
}
